...
| Node/Express 서버 | **TypeScript 기본** — `package.json`(`"type": "module"`), `tsconfig.json`(`strict: true`), 개발 `tsx watch` / 배포 `tsc` 빌드. JavaScript는 사용자가 명시적으로 요청할 때만. 계층 구조는 `conventions/express.md` 1절, 경계 검증은 **zod**. 테스트: **Vitest + supertest**, `npm test` 스크립트 등록. DB 사용 시 **마이그레이션 러너**(`npm run migrate` — migrations/ 미적용분 실행 + `schema_migrations` 기록, migration.md 5절) 포함 |
