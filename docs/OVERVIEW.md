...
- **한 호스트에 스택 공존(7-2절, 2026-07-14)**: 운영+개발을 한 PC에 띄우면 포트만으로 부족하다 — **`COMPOSE_PROJECT_NAME`을 스택별로 분리(STRICT)**. 안 하면 컨테이너·네트워크·**볼륨 이름이 겹쳐 개발 스택이 운영 DB 볼륨을 붙잡는다**(포트를 나눠도 못 막는 사고). 디렉토리·DB 자격도 분리. 외부 노출은 (a) 포트 분리(운영 80/443, 개발 8080/8443 — ⚠ 개발은 자동 HTTPS 불가: ACME는 표준 포트로만 검증) 또는 (b) 공용 프록시 + 도메인 분기(권장, 도메인 확보 시).
- 프록시: **전제 = 스택 1개 : 앱 인스턴스 1개**(기본 형태는 프로젝트별 운영·개발서버 각 1대). 앞단 리버스 프록시 1개(권장 Caddy — 자동 HTTPS), 80/443은 프록시만 노출, 정적 서빙 + `/api/*`만 서버로(CORS 원천 해소), `trust proxy` 설정. **접근 주소는 도메인으로 구분**: 사내 도메인 유무를 시작 시 확인 — 있으면 운영/`-dev` 서브도메인 + 자동 HTTPS, 없으면 `SITE_ADDRESS=:80`(확보 시 변수 교체만으로 HTTPS 전환). 운영 DB 포트 비노출(SSH 터널), 개발서버는 사내망 한정. **Caddyfile 템플릿 포함 + 스캐폴드 기본 구성**(STRICT — 없으면 prod 접근 경로가 없다), 로컬은 Vite `server.proxy`(`.env`의 API_PORT를 `loadEnv`로 — 하드코딩 금지)로 동일 출처 유지.
...
- **시작 결정 체크리스트 23항목**(+**배포 호스트 구성** — 운영·개발 분리인가 한 PC 공존인가) — 컨벤션 곳곳의 "프로젝트 시작 시 결정"(= patterns.md 0절의 "되돌리기 어려운 결정")을 모은 레지스트리 (프론트 TS/JS 명시 확인/백엔드 스택/구조/UI 모드/다크모드/브랜드 색/DB·시간대/**DB 위치**/**DB 포트**/**노출 포트**/**멀티테넌트**/**다국어**/응답 봉투/테스트 위치/**사내 도메인 유무**/CI/**가동 모니터링 — 반드시 확인**/**사내 API 연동 여부**/**인증 소스 — 반드시 확인**/**정기 배치 필요 여부**/**테스트 DB 사용 여부**/**예상 규모**). 확정값은 생성되는 CLAUDE.md에 기록.
