...
- **언어(TS/JS)는 프로젝트 시작 시 사용자에게 확인한다 (STRICT — 체크리스트 1)** — **기본값을 두지 않는다**. 중간 전환은 사실상 재생성이다. TypeScript를 택하면 `tsconfig.json`은 `strict: true` 고정, Vite `react-ts` 템플릿.
  - 선택의 판단 재료: 응답 키 계약·props 문서화·`??` vs `||` 같은 STRICT 규칙의 상당수가 TS에서는 컴파일 타임에 강제되어 규칙 의존이 줄어든다. 반대로 팀이 JS에 익숙하거나 기존 자산이 JSX면 그 이점이 학습·전환 비용을 넘지 못할 수 있다.
