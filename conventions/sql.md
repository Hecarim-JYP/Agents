...
LTMS 프로젝트에서 검증된 패턴을 범용화한 것으로, 코드 내 쿼리 함수(`*Query.js` 등)와 마이그레이션 스크립트 모두에 적용한다.
...
- 스코프 컬럼(테넌트/회사/사용자 구분값)은 **서버가 검증한 신뢰값**으로만 바인딩한다 — 클라이언트가 보낸 값을 그대로 쓰지 않는다 (근거: LTMS SEC-01 — 요청의 `company_id`를 신뢰해 타사 데이터 조회가 가능했던 보안 결함).
