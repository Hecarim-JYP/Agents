...
- **전제: 서버 1대 = 프로젝트 1개, 앱 인스턴스 1개** (2026-07-13 확정 — 프로젝트마다 운영서버 1대 + 개발서버 1대). 따라서 프록시는 프로젝트 compose에 포함되고 80/443의 주인은 이 프록시다. 한 서버에 여러 프로젝트를 올리게 되는 시점에는 이 전제가 깨진다(80/443 충돌) — 프록시를 서버 공용으로 분리하고 각 프로젝트는 external network 조인 + 앱 포트 비노출로 전환한 뒤 이 절을 갱신한다.
