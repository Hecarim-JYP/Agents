...
- 베이스 이미지는 **최소 메이저 태그로 고정**하고 `latest` 금지 (`node:22-slim`, `caddy:2-alpine`). **데이터 호환성이 걸린 이미지(DB)는 마이너까지 고정**한다 (`mariadb:11.4` — 마이너 업그레이드가 데이터 파일 포맷을 바꿀 수 있어 되돌리기 어렵다). **digest 고정(`@sha256:...`)은 하지 않는다** — 재빌드마다 베이스의 보안 패치를 자동으로 받는 편이 낫고, 배포되는 것은 CI가 빌드해 태그를 붙인 우리 이미지이므로(5절) 베이스의 rolling이 배포 산출물의 재현성을 해치지 않는다 (롤백도 재빌드가 아니라 직전 태그 이미지 pull이다).
