...
- 사용자가 **"작업 정리"**(또는 "오늘 작업을 정리해줘")를 요청하면:
  ...
  2. memory 최신화 — 확정된 사용자 선호·프로젝트 사실 반영 (중복이면 기존 파일 갱신, 코드/git으로 알 수 있는 내용은 제외)
  3. CLAUDE.md 갱신 — 새 컨벤션·Pitfall 반영, 기존 항목과 겹치면 새 번호 대신 기존 항목 갱신
  4. 백로그 동기화 — **발견 ≠ 완료 (STRICT)**: `git diff` 근거가 있는 실제 해결 건만 `☑ (YYYY-MM-DD)`, 일부 처리는 `◐`, 새 발견 이슈는 추가 (템플릿: `~/.claude/jyp/templates/backlog.md`)
  5. 기록 위치와 백로그 대조 결과를 요약 보고. 커밋/푸시는 명시 요청 시에만
- 사용자가 **"배포 준비"**를 요청하면: 테스트 전체 실행 → 프로덕션 빌드 → `npm audit` → 미적용 마이그레이션 확인 → `.env.example` 대조 → ops.md 5절 보안 체크리스트 → changelog 최신 확인 → main/태그 확인. 각 항목 ✅/❌/– 표로 보고, 실패 있으면 배포 불가. 실제 배포는 사용자 승인 후에만
