...
| `conventions/` | 코딩 컨벤션 — general(범용), patterns(구현 패턴), sql, database(스키마), express, react, design(UI/UX·테마), api(REST 설계), auth(인증·권한), testing, migration(DB 변경), ops(배포·운영), docker(컨테이너·프록시) |
...
| `schemas/` | DB 기초 테이블 표준 DDL — 인증·권한, 공통코드, 파일 메타, 감사 로그, schema_migrations |
